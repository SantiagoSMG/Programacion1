...
style.configure("Sidebar.TButton", font=("Segoe UI", 11, "bold"), foreground="#FFFFFF", background="#E50914", padding=8, relief="flat")
style.map("Sidebar.TButton", background=[("active", "#B20710")])
style.configure("Genero.TButton", font=("Segoe UI", 10, "bold"), foreground="#FFFFFF", background="#6A1BB8", padding=8)
style.map("Genero.TButton", background=[("active", "#8A2DDB")])
style.configure("Item.TButton", font=("Segoe UI", 10), foreground="#FFFFFF", background="#13a3c7", padding=6)
...
def open_favorites_window():
    ...
    if not favs:
        tk.Label(list_frame, text="No hay favoritos aún.", bg="#121212", fg="#ccc", font=("Segoe UI", 12)).pack(pady=20)
    else:
        for t in favs:
            row = tk.Frame(list_frame, bg="#121212")
            row.pack(fill="x", pady=4)
            ttk.Button(row, text=t, style="Item.TButton", command=lambda tt=t: __open_review_from_win(tt, win)).pack(side="left", padx=6)
            star = "★" if is_favorite(t) else "☆"
            ttk.Button(row, text=star, command=lambda tt=t, parent=win: _toggle_and_refresh(tt, parent)).pack(side="right", padx=6)
...
ttk.Button(sidebar, text="Favoritos", style="Sidebar.TButton", command=open_favorites_window).pack(fill="x", padx=12, pady=6)
