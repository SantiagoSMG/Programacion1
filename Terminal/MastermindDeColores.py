    ...
    while True:
        ...
        for letra in intento:
            if letra not in COLORES_POSIBLES:
                print(Fore.RED+f"Error: La letra '{letra}' no es color valido. "+Style.RESET_ALL)
                valido = False
                break
...
def mostrar_tablero(intentos_pasado,pistas_pasadas):
    """Mostrar el historial de intentos con sus pistas"""
    print("\n--- Tablero del juego ---")
    for i in range(len(intentos_pasado)):
        intento_str =""
        for letra in intentos_pasado[i]:
...
def jugar():
    ...
    while intentos_restantes > 0:
        print(f"Te quedan {Fore.YELLOW+Style.BRIGHT}{intentos_restantes}{Style.BRIGHT} intentos")
